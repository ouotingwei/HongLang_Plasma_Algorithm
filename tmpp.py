...
def pointCloudSample(diameter):
    ...
    PointArray = np.zeros((filterCNT, 6), float)
    ...
    while i < len(pcdSample_pre):
        if pcdSample_pre[i][6] == 1:
            PointArray[pcdCNT][0] = pcdSample_pre[i][0]
            PointArray[pcdCNT][1] = pcdSample_pre[i][1]
            PointArray[pcdCNT][2] = pcdSample_pre[i][2]
            PointArray[pcdCNT][3] = pcdSample_pre[i][3]
            PointArray[pcdCNT][4] = pcdSample_pre[i][4]
            PointArray[pcdCNT][5] = pcdSample_pre[i][5]
...
def BottomFlat(gate):
    ...
    i = 0
    while i < len(PointArray):
        PointArray[i][0] = PointArray[i][0]
        PointArray[i][1] = PointArray[i][1]
        PointArray[i][2] = PointArray[i][2]
...
def wallNormalProccessing(d, CountingArray):
    ...
    for i in range(len(Point)-1):
        if(CountingArray[i][3] <= corner and CountingArray[i][3] > -corner ):
            ...
            Parall[i][0] = 0.000
            Parall[i][1] = -45 # 90 - slope*180/3.1415
            Parall[i][2] = 0.000
        ...
        if(CountingArray[i][3] <= (180 - corner) and CountingArray[i][3] > corner ):
            ...
            Point[i][2] = Point[i][2] + d*math.cos(slope)
            Parall[i][0] = 45 # - (90 - slope*180/3.1415)
            Parall[i][1] = 0.000
            Parall[i][2] = 0.000
...
def workingSpaceTF(Position,Vector):
    ...
    transition_v = [90, 0, 180 -  theta*(180/3.1415)]
...
def main():
    ...
    # !
    OutputFile = "WALL01D15.LS"
    Wall(gate)
    writeLsFile(OutputFile, waypoints)

    OutputFile = "BOTTOM01D15.LS"
    BottomFlat(gate)
    writeLsFile(OutputFile, waypoints)
